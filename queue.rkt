...
(define (queue-empty? q)
  (if (and (stream-empty? (queue-left q)) (null? (queue-right q)))
      #t
      #f))
...
(define (enqueue x q)
  (match q [(queue left right size-l size-r)
            (if (= size-r size-l)
                (struct-copy queue q [left (rotate left (cons x right) empty-stream)][size-l (add1 (+ size-r size-l))][right '()][size-r 0])
                (struct-copy queue q [right (cons x right)][size-r (add1 size-r)])
                )
            ]))
; TODO
...
(define (dequeue q)
  (match q [(queue left right size-l size-r)
            (if (= size-r size-l)
                (struct-copy queue q [left (rotate (stream-rest left) right empty-stream)][size-l (sub1 (+ size-r size-l))][right '()][size-r 0])
                (struct-copy queue q [left (stream-rest left)][size-l (sub1 size-l)])
                )
            ]))
; TODO
